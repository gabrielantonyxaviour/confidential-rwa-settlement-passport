...
import { DEMO_SETTLEMENT_AMOUNT, TRUSTED_VERIFIER_HASH_HEX } from "./lib/config";
import { getBalance, recordPass, settle } from "./lib/contract";
import { getErrorMessage, normalizeContractError } from "./lib/errors";
import { generateSettlementProof, type SettlementProofResult } from "./lib/proof";
import { disconnectWallet, openWalletModal, type SignTransaction } from "./lib/wallet-kit";
...
export default function App() {
  ...
  const [recordPassStatus, setRecordPassStatus] = useState<TxStatusState>({ type: "idle" });
  const [settleStatus, setSettleStatus] = useState<TxStatusState>({ type: "idle" });
  ...
  const refreshBalance = useCallback(async (walletAddress: string) => {
    ...
    try {
      ...
      setBalanceError(getErrorMessage(error) || "Could not load the settlement balance.");
    } finally {
  ...
  async function handleProveAndRecord() {
    if (!address || !signTransaction) {
      setRecordPassStatus({ type: "error", message: "Connect Freighter before proving eligibility." });
      return;
    }

    let nextProof: SettlementProofResult;
    setIsProofGenerating(true);
    setRecordPassStatus({ type: "pending", message: "Generating the Noir proof in this browser..." });
    ...
    try {
      ...
      setRecordPassStatus({
        type: "error",
        message: getErrorMessage(error) || "Proof generation failed in the browser.",
      });
    ...
    setRecordPassStatus({
      type: "pending",
      message: "Proof generated. Waiting for Freighter and Stellar confirmation...",
    });
  ...
  async function handleSettle() {
    if (!address || !signTransaction || !proof) {
      setSettleStatus({ type: "error", message: "Record an eligibility pass before settling." });
      return;
    }
    ...
    if (proof.buyerAddress !== address.trim()) {
      setSettleStatus({
        type: "error",
        message: "Generate an eligibility pass for the connected wallet before settling.",
      });
      return;
    }

    setIsSettling(true);
    setSettleStatus({ type: "pending", message: "Waiting for Freighter and Stellar settlement confirmation..." });
    ...
    try {
      ...
      setSettleStatus({ type: "error", message: normalizeContractError(error, "settlement") });
    } finally {
      setIsSettling(false);
    }
  }
  ...
  return (
    <main className="app-shell">
      ...
      <BalanceDisplay address={address} balance={balance} isLoading={isBalanceLoading} error={balanceError} />
