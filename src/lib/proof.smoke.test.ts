...
const BUYER_ADDRESS_A =
  "GAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAWHF";
const BUYER_ADDRESS_B =
  "GBL32CFUIH4WVIBYHHEW7HQBZ5M3XGOBGGLDULAK6AI3RR6NQ7RWAONE";

describe("generateSettlementProof", () => {
  it("derives distinct action-scoped nullifiers for distinct buyers", async () => {
    const circuit = circuitArtifact as CompiledCircuit;
    const first = await generateSettlementProofFromCircuit(
      circuit,
      BUYER_ADDRESS_A,
    );
    const second = await generateSettlementProofFromCircuit(
      circuit,
      BUYER_ADDRESS_B,
    );

    expect(first.proof.length).toBe(14_592);
    expect(second.proof.length).toBe(14_592);
    ...
    expect(first.credentialRoot).toBe(
      DEMO_CREDENTIAL_INPUTS.credential_root.slice(2),
    );
    expect(second.credentialRoot).toBe(
      DEMO_CREDENTIAL_INPUTS.credential_root.slice(2),
    );
